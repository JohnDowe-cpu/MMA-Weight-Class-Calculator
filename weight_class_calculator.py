...
def get_weight_class(weight: int) -> str | None:
    """Return the MMA weight class for the given weight."""
...
def get_weight() -> int:
    """Prompt the user to enter a fighter's weight."""
...
def get_fighter_name() -> str:
    """Prompt the user to enter a fighter's name."""
...
def save_fighter_profile(fighter: dict) -> None:
    """Save a fighter profile to a file."""

    with open(FIGHTERS_FILE, "a", encoding="utf-8") as file:
...
def view_saved_fighters() -> None:
    """Display all saved fighter profiles."""
...
def create_fighter_profile() -> None:
    """Create, display, and save a new fighter profile."""
    ...
    if weight_class:
        ...
        print(f"Fighter profile saved to {FIGHTERS_FILE}.")
    else:
        print("Sorry! You are not heavy enough for a listed weight class yet.")


def search_fighter_by_name() -> None:
    """Search for a fighter by name in the saved profiles."""
